...
doc.autoTable({
  ...
  body: materiales.map(m => {
    ...
    return [
      m.tipo,
      m.esFabrica ? '[FABRICA] ' + m.ref : m.ref,
      m.descripcion,
      m.acabado,
      refAcabado,
      longBarra,
...
if (hayPerfilesFabrica) {
  const notaFabricaText = 'ATENCION: Los perfiles marcados como [FABRICA] superan la medida maxima de almacenamiento (6,50 m) y deben pedirse a fabrica con medida especial (longitud neta + 10 cm de descarte).';
  const notaLines = doc.splitTextToSize(notaFabricaText, contentWidth - 8);
  const notaH = Math.max(12, notaLines.length * 4 + 6);
  ...
  filas.forEach(tr => {
    ...
    if (celdas.length >= 9) {
      materiales.push({
        ...
        ref: celdas[1].textContent.trim().replace('⚠️ ', ''),
        descripcion: celdas[2].textContent.trim().replace('[FÁBRICA]', '').trim(),
...
function generarFilasTabla(filas) {
  ...
  const filasHtml = filas.map(item => {
    ...
    const refCell = `${esF ? '⚠️ ' : ''}${item.ref || '—'}`;
    const descCell = `${item.descripcion || '—'}${esF ? ' <span style="color:#b45309;font-size:0.75em;font-weight:700;">[FÁBRICA]</span>' : ''}`;
    return `
  ...
  const notaFabrica = hayFabrica ? `
    <tr>
      <td colspan="9" style="
        ...
        font-style:italic;
      ">
        ⚠️ Los perfiles marcados como <strong>[FÁBRICA]</strong> superan la medida máxima de almacenamiento.
        Deben pedirse a fábrica con medida especial (longitud neta + 10 cm de descarte).
      </td>
    </tr>
  ` : '';
