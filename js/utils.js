...
import { LIMITE_SALIDA_M, LIMITE_ANCHO_MODULO_M } from './config.js';
...
/**
 * Evalúa si alguna dimensión está fuera de rango y devuelve los avisos correspondientes.
 * También detecta si las medidas superan los límites físicos absolutos (imposible).
 * @param {object} params - {salida, ancho, modulos, altura}
 * @returns {object} { esImposible: boolean, fueraDeRango: boolean, avisos: string[] }
 */
export function validarDimensionesAviso({ salida, ancho, modulos, altura }) {
  const avisos = [];

  // ── Límites físicos absolutos (bloqueo total) ────────────────────────────
  const anchoPorModulo = (ancho !== null && ancho !== undefined && modulos > 0)
    ? ancho / modulos
    : null;
  const sExcede = salida !== null && salida !== undefined && salida > LIMITE_SALIDA_M;
  const aExcede = anchoPorModulo !== null && anchoPorModulo > LIMITE_ANCHO_MODULO_M;
  const esImposible = sExcede || aExcede;

  // ── Rangos habituales (advertencia, no bloqueo) ──────────────────────────
  if (!esImposible) {
    if (salida !== null && salida !== undefined) {
      if (salida < 1.5 || salida > 6.0) {
        avisos.push(`Salida ${salida.toFixed(2)} m fuera del rango habitual (1,50 – 6,00 m)`);
      }
    }

    if (anchoPorModulo !== null) {
      if (anchoPorModulo < 1.5 || anchoPorModulo > 4.0) {
        avisos.push(`Ancho por módulo ${anchoPorModulo.toFixed(2)} m fuera del rango habitual (1,50 – 4,00 m)`);
      }
    }

    if (altura !== null && altura !== undefined && altura > 0) {
      if (altura < 2.0 || altura > 2.8) {
        avisos.push(`Altura libre ${altura.toFixed(2)} m fuera del rango habitual (2,00 – 2,80 m)`);
      }
    }
  }
