...
export function optimizarBarras(ref, piezasMm) {
  ...
  if (!perfil || !piezasMm || !piezasMm.length) {
    return { barrasPorLongitud: {}, desperdicioTotal: 0, barrasDetalle: [], esFabrica: false };
  }
  ...
  const maxStockMm = longitudes.length > 0 ? longitudes[longitudes.length - 1] : 0;
  const maxPiezaMm = piezasMm.length > 0 ? Math.max(...piezasMm) : 0;

  // ── PERFIL DE FÁBRICA ──────────────────────────────────────────────────────
  // Si alguna pieza supera la barra más larga en stock, se pide a fábrica.
  // La longitud de suministro = longitud neta de la pieza + DESCARTE_FABRICA_MM.
  // No se optimiza: cada pieza es una barra individual.
  if (maxStockMm > 0 && maxPiezaMm > maxStockMm) {
    const barrasPorLongitud = {};
    const barrasDetalle = [];

    piezasMm.forEach(pieza => {
      const longitudFabrica = pieza + DESCARTE_FABRICA_MM;
      barrasPorLongitud[longitudFabrica] = (barrasPorLongitud[longitudFabrica] || 0) + 1;
      barrasDetalle.push({
        longitud: longitudFabrica,
        piezas: [pieza],
        desperdicio: DESCARTE_FABRICA_MM,
        capacidadRestante: DESCARTE_FABRICA_MM,
        esFabrica: true
      });
    });
    ...
    return {
      barrasPorLongitud,
      desperdicioTotal: piezasMm.length * DESCARTE_FABRICA_MM,
      barrasDetalle,
      esFabrica: true
    };
  }
  ...
  // Calcular desperdicio por cada barra para el detalle
  const barrasDetalle = solucion.barras.map(barra => {
    ...
    const desperdicio = barra.longitud - sumaPiezas - mermaTotal - margenTotal;
    
    return {
      longitud: barra.longitud,
      piezas: [...barra.piezas],
      desperdicio: desperdicio,
      capacidadRestante: barra.capacidadRestante,
      esFabrica: false
    };
  });
...
export function calcularPerfiles(piezasPerfiles, config) {
  ...
  Object.entries(piezasPerfiles || {}).forEach(([ref, piezas]) => {
    ...
    if (opt.esFabrica) perfilesFabrica.add(ref);
...
export function calcularPrecios(materiales, ctx, config) {
  ...
  return {
    ...
    detalle,
    perfilesFabrica: perfiles.perfilesFabrica
  };
}